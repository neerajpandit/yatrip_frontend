...
const Madhya = () => {
    const navLinks = [
        { icon: <AiFillAppstore className='text-2xl lg:text-[28px]' />, label: 'Overview', route: '/MadhyaPradeshOverview' },
        { icon: <LiaBrushSolid className='text-2xl lg:text-3xl' />, label: 'Cities', route: '/MadhyaPradeshCities' },
        { icon: <FaImage className='text-2xl lg:text-[26px]' />, label: 'Gallery', route: '/MadhyaPradeshGallery' }
    ];
                            ...
                            <div className='grow bg-transparent md:flex flex-col justify-between my-1 md:my-0 rounded-[12px] font-SulphurPoint'>
                                <div className='my-2 md:my-0 bg-[#FFFFFF] rounded-xl'>
                                    <div className='flex md:flex-col justify-between items-center md:items-start p-2 md:p-1'>
                                        <h4 className='font-bold text-[#2D2D2D] md:text-[#A21D3C] text-xl md:text-2xl lg:text-3xl xl:text-4xl'>Madhya Pradesh</h4>
                                ...
                                <div className='bg-[#FFFFFF] md:bg-[#F3EFEF] rounded-[12px] overflow-hidden flex justify-between items-center md:gap-4'>
                                    {/* Mapping over navLinks array to render each NavLink */}
                                    {navLinks.map((navLink, index) => (
                                        <NavLink
                                            key={index}
                                            to={navLink.route}
                                            className={({ isActive }) => `${isActive ? 'text-[#D66F11]': 'text-[#989898]' } flex flex-col p-2 rounded-xl bg-[#FFFFFF] items-center w-full justify-center md:shadow-ArtistDetailsCard cursor-pointer xl:h-[80px] text-4xl font-semibold`}
                                        >
                                            {navLink.icon}
                                            <p className='font-DMSans text-base lg:text-lg xl:text-xl'>{navLink.label}</p>
                                        </NavLink>
                                    ))}
                                </div>
                            </div>
